...
$query = "SELECT * FROM pages WHERE page_name='projects_it'";
...
while ($row = mysqli_fetch_assoc($result)){
    ...
    $filters .= <<<HTML
<li class="filter_item $classe_active" data-filter="{$row['prof_filter']}"><a class="filter_link" href="#" title="{$row['prof_name_it']}">{$row['prof_name_it']}</a></li>
...
<figure class="gallery-grid_item {$row['pro_cat']}">
    <div class="gallery-grid_image-wrap">
        <a href="{$row['pro_link']}" title="{$row['pro_alt_it']}" rel="noreferrer" target="_blank"><img class="gallery-grid_image cover" src="{$row['pro_attachment']}" data-zoom alt="{$row['pro_alt_it']}"/></a>
    </div>
    <figcaption class="gallery-grid_caption">
        <h4 class="title title-h4 gallery-grid_title">{$row['pro_name_it']}</h4>
        <span class="gallery-grid_category">{$row['pro_desc_it']}</span>
    </figcaption>
</figure>
        ...
        <div class="wrapper">
            ...
            <div class="content" data-simplebar>
                <div class="section mt-0">
                    <h2 class="title title-h1 title_separate">Progetti</h2>
                </div>

                <div class="select section">
                    <span class="placeholder">Seleziona categoria</span>
                    <ul class="filter">
                        <li class="filter_item">Categorie</li>
                        $filters
                    </ul>
                </div>
